...
4. 各C-01〜C-05およびE-01〜E-04について、`ID / 宣言済みの評価観点 / テストレベル（Eのみ） / Pass|Fix|Needs Review / 根拠 / 不合格時の再現手順` を返す。定義されていない観点は評価しない。
5. 返答全文を `evaluator-result.md` にも保存する。

高影響操作の不明点があればNeeds Reviewにしてください。今回のPoCで高影響操作は対象外です。
